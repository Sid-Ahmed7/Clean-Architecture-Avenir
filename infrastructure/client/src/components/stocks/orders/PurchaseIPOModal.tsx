...
export function PurchaseIPOModal({isOpen,onClose,stockSymbol,stockName,ipoPrice,availableShares,}: PurchaseIPOModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (quantity > availableShares) {
      alert(
        `Seulement ${availableShares} actions disponibles en IPO`
      );
      return;
    }
    ...
    try {
      ...
      alert(` ${error.message || "Erreur lors de l'achat IPO"}`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <div className="mb-4 p-4 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-green-900">Introduction en Bourse</p>
              <p className="text-sm text-green-800 mt-1">
                Achat direct au prix d&apos;émission initial
              </p>
              <div className="mt-2 text-xs text-green-700">
                <span className="font-semibold">{availableShares}</span> actions restantes
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Maximum : {availableShares} actions disponibles
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-blue-50 p-4 rounded-lg space-y-2 border border-green-200">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-gray-700">Frais de transaction</span>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-xs text-blue-800">
              Votre achat sera exécuté instantanément au prix IPO. Les actions seront ajoutées directement à votre portefeuille.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              {isLoading ? (
                ...
                `Acheter ${quantity} action${quantity > 1 ? "s" : ""} IPO`
              )}
            </button>
          </div>
        </form>
