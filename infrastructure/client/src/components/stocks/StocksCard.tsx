    ...
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-xl transition-shadow">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded ${stock.isActionAvailable
            ...
            {isIPOActive && (
              <span className="text-xs px-2 py-0.5 bg-gradient-to-r from-green-100 to-blue-100 text-green-800 rounded font-semibold animate-pulse">
                {t("ipoActive", { count: stock.availableSharesForIPO })}
              </span>
            )}
          </div>
        </div>
      ...
      {stock.previousPrice && (
        <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
          <div>
            <div className="text-gray-500">{t("previousPrice")}</div>
      ...
      {stock.isActionAvailable && (
        <div>
          {isIPOActive && onBuyIPO && (
            <div className="mb-3 p-3 bg-gradient-to-r from-green-50 to-blue-50 border-2 border-green-300 rounded-lg">
              ...
              <p className="text-xs text-green-700 mb-2">
                {t("ipoDesc", { price: stock.currentPrice.toFixed(2) })}
              </p>
              <button
              ...
              >
                {t("buyIpo", { count: stock.availableSharesForIPO })}
              </button>
            </div>
          )}
          ...
          {!isIPOActive && (
            <div className="flex gap-2 mb-2">
              {onBuy && (
                ...
                >
                  {t("buyAt", { price: stock.currentPrice.toFixed(2) })}
                </button>
              )}
              {onSell && (
                <button
          ...
          <div className="mt-3 text-xs text-center text-gray-500">
            {t("updatedAt")} {new Date(stock.updatedAt).toLocaleString(locale)}
          </div>
        </div>
      )}
