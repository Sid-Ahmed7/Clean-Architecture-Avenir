...
export function AccountSummary({ account }: AccountSummaryProps) {
    ...
    return (
        <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold mb-4">Répartition du solde</h2>
            ...
            <div className="flex justify-between gap-4 text-sm">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-700">
                        Disponible <span className="font-semibold">({availableBalance.toFixed(2)} {account.currency})</span>
                    </span>
                </div>
                {blockedBalance > 0 && (
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700">
                            Bloqué <span className="font-semibold">({blockedBalance.toFixed(2)} {account.currency})</span>
                        </span>
                    </div>
                )}
            </div>
            ...
            {blockedBalance > 0 && (
                <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                    <p className="text-sm text-orange-800">
                        <span className="font-semibold">{blockedBalance.toFixed(2)} {account.currency}</span> sont actuellement bloqués dans vos ordres d&apos;achat en attente.
                    </p>
                </div>
            )}
        </div>
