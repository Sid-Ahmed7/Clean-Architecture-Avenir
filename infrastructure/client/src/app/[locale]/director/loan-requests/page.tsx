                ...
                .map((req) => {
                  ...
                  return (
                    <React.Fragment key={req.id}>
                      ...
                      {clientDetails[req.clientId] && (
                        <tr>
                          <td colSpan={6} className="bg-gray-50 px-4 py-3 text-sm text-gray-800">
                            <div className="font-semibold mb-1">Profil client</div>
                            <div className="mb-2">
                              {clientDetails[req.clientId].user
                                ? `${clientDetails[req.clientId].user.firstName ?? ""} ${clientDetails[req.clientId].user.lastName ?? ""}`.trim()
                                : ""}
                            </div>
                            <div>
                              <span className="font-medium">Comptes :</span>
                              <ul className="list-disc list-inside">
                                {(clientDetails[req.clientId].accounts ?? []).map((acc: any) => (
                                  <li key={acc.accountNumber}>
                                    {acc.accountType} - {acc.accountNumber} : {acc.currentBalance} {acc.currency}
                                  </li>
                                ))}
                              </ul>
                            </div>
